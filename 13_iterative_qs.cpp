...
void q_sort(int a[], int p, int q) {
    stack<int> s1; // Fix: Specify the type of stack elements
    q = q - 1;
    while (true) { // Fix: Changed 1 to true for readability
        while (p < q) {
        ...
        if (s1.empty()) { // Fix: Changed is_empty() to empty() for stack
            return;
        }
        q = s1.top();
        s1.pop();
